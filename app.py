    ...
    # — BUSCAR —
        ...
        if st.button("Cargar datos", key="load_edit"):
            ...
            if not row:
                ...
                # guardamos el registro en session_state
        ...
        if st.session_state.get(load_flag, False):
            ...
            with st.form("form_edit", clear_on_submit=True):
                # Cada widget con su key preserva valor tras rerun
                ...
                valor_unitario2  = st.number_input(
                                    "Valor Unitario",
                                    min_value=0.0,
                                    format="%.2f",
                                    value=float(data["valor_unitario"]),
                                    key="valor2"
                                )
                fecha_i2         = st.date_input(
                                    "Fecha Inicio de Proceso",
                                    value=pd.to_datetime(data["fecha_inicio"]).date(),
                                    key="fecha2"
                                )
                enviar2 = st.form_submit_button("Actualizar")

            # 3) Al enviar, actualizamos DB y session_state
            if enviar2:
                ...
                try:
                    ...
                    """, (
                        ...
                        conc2, desc2, cantidad2, valor_unitario2, fecha_i2,
                        id_ed
                    ))
                    cn3.commit()
                ...
                # 5) Limpio caché y fuerzo recarga del sidebar
                try:
                    load_ingresos.clear()
                    load_gastos.clear()
                except:
